...
st.markdown('<div class="card"><h3>Total Bill vs Tip (with MLR)</h3>', unsafe_allow_html=True)
fig, ax = plt.subplots()
...
st.markdown(f"""
    <div class="card">
        ...
        <p><b> Co-efficient (Total Bill): </b>{m1:.3f}<br>
        <b> Co-efficient (Size): </b>{m2:.3f}<br>
        <b>Intercept: </b>{c:.3f}</p>
        <p>Tip depends on <b>Total Bill</b> and <b>Size</b></p>
    </div>
...
bill = st.slider("Total Bill Amount ($):",
    min_value=min_bill,
    max_value=max_bill,
...
st.markdown(f'<div class="prediction-box"><h3>Predicted Tip for Total Bill ${bill:.2f} and Size {size} is <b>${tip:.2f}</b></h3></div>', unsafe_allow_html=True)